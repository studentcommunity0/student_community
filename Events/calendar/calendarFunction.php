...
function getEvents($date = ''){ 
    ...
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){ $i++; 
            $id = $row['id'];
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">  '.$i.'.  </span>'.$row['name'].'   [<b>'.$row['type'].' </b>] <a class="eventBtn" href="../singleEvent.php?EventID='.$id.'">view event</a></li>';
        ...
        $eventListHTML .= '<hr>'; 
        $eventListHTML .= '<ul class="sidebar__list">'; 
        $eventListHTML .= '<li class="sidebar__list-item sidebar__list-item--Notcomplete">No events</li>';
    }
    echo $eventListHTML; 
}
